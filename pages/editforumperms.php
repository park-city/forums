...
$title = __('Forum permissions editor');
...
$crumbo = array(__('Admin') => actionLink('admin'));
$crumbo[__('Edit permissions for forum: ').htmlspecialchars($forum['title'])] = '';
...
if ($_POST['saveaction'])
{
	...
	die(header('Location: '.actionLink('editforumperms', '', 'id='.$id)));
}
...
$fperms = Query("SELECT p.*, g.id gid, g.name gname FROM {usergroups} g LEFT JOIN {permissions} p ON p.id=g.id AND p.applyto=0 AND p.arg={2}
